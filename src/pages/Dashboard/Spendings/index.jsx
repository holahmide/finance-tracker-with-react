...
const Spendings = () => {
  ...
  } = usePagination({ data: spendings, size: 10, span: 10 });
  ...
  const addSpending = useCallback((data) => {
    const spendingRecord = data.spending;
  ...
  const updateSpending = useCallback((data) => {
    ...
    spendingRecord.Lents = data.Lents;
    setSpendings((oldValue) => {
      const findIndex = oldValue.findIndex((el) => el.id === Number(spendingRecord.id));
    ...
    <div>
      ...
      {deleteModal && (
        <Modal closeModal={hideDeleteModal}>
          ...
          <div className="mt-3">
            Are you sure you want to remove this spent record of ₦{' '}
            {activeDeleteSpending.amount.toLocaleString()} on{' '}
            <Moment format={dateFormat}>{activeDeleteSpending.date}</Moment>?
          </div>
          <div className="flex justify-end mt-4">
            <div>
              ...
              <button
                onClick={() => deleteSpending(activeDeleteSpending.id)}
                ...
                {tableData.map((spending, index) => (
                  <tr className="text-center" key={index}>
                    ...
                    <td className="border-b">
                      <Moment format={dateFormat}>{spending.date}</Moment>
                    </td>
