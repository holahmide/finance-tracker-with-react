...
const Borroweds = () => {
  ...
  } = usePagination({ data: borroweds, size: 10, span: 10 });
  ...
  useEffect(() => {
    const initialSetup = async () => {
      try {
        ...
        // if(Number(page)) goToPage(() => Number(page));
      } catch (error) {
        toast(error?.response?.data?.message || "Couldn't fetch borrowed record. Reload the page.");
      } finally {
        setLoading(() => false);
      }
    };
  ...
  const addBorroweds = useCallback((data) => {
    const borrowedRecords = data.borroweds;
  ...
  const updateBorrowed = useCallback((data) => {
    const borrowedRecord = data.borrowed;
    setBorroweds((oldValue) => {
      const findIndex = oldValue.findIndex((el) => el.id === Number(borrowedRecord.id));
      ...
      <div>
        ...
        {deleteModal && (
          <Modal closeModal={hideDeleteModal}>
            ...
            <div className="mt-3">
              Are you sure you want to remove this borrowed record of ₦{' '}
              {activeDeleteBorrowed.amount.toLocaleString()} on{' '}
              <Moment format={dateFormat}>{activeDeleteBorrowed.date}</Moment>?
            </div>
            <div className="flex justify-end mt-4">
              <div>
                ...
                <button
                  onClick={() => deleteBorrowed(activeDeleteBorrowed.id)}
                ...
                <tbody>
                  {tableData.map((borrowed, index) => (
                    <tr className="text-center" key={index}>
                      ...
                      <td className="border-b">₦ {borrowed.amount.toLocaleString()}</td>
                      <td className="border-b">
                        <Moment format={dateFormat}>{borrowed.date}</Moment>
                      </td>
                      <td className="border-b">{borrowed.description}</td>
              ...
              <div className="flex justify-left">
                <form onSubmit={quickAdd} className="">
                  ...
                  <CreateSpendingAction
                    type={'borrowed'}
                    addSpendingAction={addBorroweds}
                    setRequestState={setRequestState}
                  />
                </form>
              </div>
