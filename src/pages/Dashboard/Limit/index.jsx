...
const Limit = () => {
  ...
  } = usePagination({ data: overshoots, size: 10, span: 10 });
  ...
  const initialSetup = useCallback(async () => {
    try {
      ...
      toast(
        error?.response?.data?.message || "Couldn't fetch limits & overshoots. Reload the page."
      );
    } finally {
      setLoading(() => false);
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-6 w-full justify-around gap-6">
                  ...
                  <tr
                    className={`text-center ${
                      overshoot.credit
                        ...
                        : 'bg-red-900 dark:bg-red-900 text-white'
                    }`}
                    key={index}>
                    <td className="border-b p-2 border-r">{tableLastIndex + (index + 1)}</td>
                    <td className="border-b">₦ {overshoot.amount.toLocaleString()}</td>
                    <td className="border-b">
                      {overshoot.credit ? '+' : '-'} ₦{overshoot.difference.toLocaleString()}
                    </td>
                    <td className="border-b capitalize">{overshoot.type}</td>
                    <td className="border-b">
                      <Moment format={dateFormat}>{overshoot.date}</Moment>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="md:col-span-2">
          <div className="bg-gray-300 dark:bg-dark rounded-sm w-full p-4 mb-3">
            <div className="text-left dark:text-primary text-main mb-2">Active Limit</div>
