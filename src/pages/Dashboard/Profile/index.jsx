                ...
                <div className="dark:bg-dark bg-gray-300 p-5 w-full">
                    <form onSubmit={updateProfile} className="flex flex-wrap gap-4 w-full">
                        <div className="md:w-1/4 w-full text-left">
                            ...
                            <input required value={firstname} onChange={firstNameChangeHandler} className={`${formInputClass}`} type="text" />
                        </div>
                        <div className="md:w-1/4 w-full text-left">
                            <label htmlFor="lastname">Lastname *</label><br />
                            <input required value={lastname} onChange={lastNameChangeHandler} className={`${formInputClass}`} type="text" />
                        </div>
                        <div className="md:w-1/4 w-full text-left">
                            <label htmlFor="email">Email *</label><br />
                            <input required value={email} onChange={emailChangeHandler} className={`${formInputClass}`} type="email" />
                        </div>

                        <div className="w-full text-left">
                            <button disabled={updateState} className="dark:bg-primary bg-main text-white px-4 py-2 rounded-md hover:opacity-90 disabled:cursor-not-allowed">{updateState ? 'Updating' : 'Update'}</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div className="dark:bg-dark bg-gray-300 p-5">
                <form onSubmit={changePassword} className="flex flex-wrap gap-4">
                    <div className="md:w-1/4 w-full text-left">
                        ...
                        <input required value={currentPassword} onChange={currentPasswordChangeHandler} className={`${formInputClass}`} type="password" />
                    </div>
                    <div className="md:w-1/4 w-full text-left">
                        <label htmlFor="new_password">New Password *</label>
                        <input required value={newPassword} onChange={newPasswordChangeHandler} className={`${formInputClass}`} type="password" />
                    </div>
                    <div className="md:w-1/4 w-full text-left">
                        <label htmlFor="confirm_new_password">Confirm Password *</label>
                        <input required value={confirmNewPassword} onChange={confirmNewPasswordChangeHandler} className={`${formInputClass}`} type="password" />
                    </div>

                    <div className="w-full text-left">
                        <button disabled={changePasswordState} className="dark:bg-primary bg-main text-white px-4 py-2 rounded-md hover:opacity-90 disabled:cursor-not-allowed">{changePasswordState ? 'Changing...' : 'Change'}</button>
                    </div>
                </form>
            </div>
        </div>
