...
const useSpending = (user) => {
  ...
  const addFields = useCallback(
    (type = 'Breakdowns', length = 1) => {
      const newFields = [];
      const formField = Object.assign({}, fields[type]);
  ...
  const inputChangeHandler = useCallback((value, pageId, type, field, id = null) => {
    setSpending((oldValue) => {
      let findIndex = -1;
      if (id) {
        // For old records of spending, that wont have a pageId
        findIndex = oldValue[type].findIndex((el) => Number(el.id) === id);
      } else {
        // For new record assingned a page number
    ...
    (form) => {
      ...
      for (i = 0; i < breakdowns.length; i++) {
        if (breakdowns[i].price) {
          ...
          minor_errors.push(
            'You provided an item for breakdown in field ' + (i + 1) + ' of the table'
          );
        } else {
      ...
      const borroweds = form.borroweds;
  ...
  const validateSpendingAction = useCallback((form) => {
    if (form.amount) {
      ...
      if (form.description) {
        toast('You provided a description for an amount(not specified) you lent');
        return false;
      }
      if (form.repay_date) {
        toast('You provided a repay_date for an amount(not specified) you lent');
        return false;
      }
    } else {
