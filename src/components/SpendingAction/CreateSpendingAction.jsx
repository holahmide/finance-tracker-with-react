...
const CreateSpendingAction = ({ type, addSpendingAction, setRequestState }) => {
    ...
    const recordSpendingAction = async () => {
        let form = spending[`${capitalizeFirstLetter(type)}s`];
        ...
        <div>
            ...
            {showModal && (
                <Modal closeModal={closeModal}>
                    <div>
                        <div className="text-left font-bold text-lg">Create {capitalizeFirstLetter(type)} Record(s)</div>
                        <input
                            ...
                            {spending[`${capitalizeFirstLetter(type)}s`].map((spendingAction, index) => (
                                <div key={index}>
                                    <div className="mb-2 text-left">Item {index + 1} &nbsp;
                                        <FaTrash
                                            onClick={() => deleteField(`${capitalizeFirstLetter(type)}s`, spendingAction.pageId, spendingAction.id)}
