...
const CreateLent = ({ addLent, setRequestState }) => {
    ...
    const recordLent = async () => {
        let form = spending.Lents;

        setRequestState(() => true)

        try {
            const response = await SpendingActionsService.createLent(form);
            if (response.status === 201) {
                setShowModal(() => false);
                toast.success("Successfully added your lent record(s)");
                addLent(response.data);
                reset();
            }
        } catch (error) {
        ...
        <div>
            ...
            {showModal && (
                <Modal closeModal={closeModal}>
                    <div>
                        <div className="text-left font-bold text-lg ">Create Lent Record(s)</div>
                        <input
