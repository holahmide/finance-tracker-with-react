...
const EditSpendingAction = ({ type, spendingAction: lentData, close, updateSpendingAction, setRequestState }) => {
    ...
    const editSpendingAction = async () => {
        ...
        try {
            ...
            if (type.toLowerCase() === 'borrowed') {
                response = await SpendingActionsService.editBorrowed(
                    form,
                    form.id
                );
            } else {
                response = await SpendingActionsService.editLent(
                    form,
                    form.id
                );
            }
        ...
        <div>
            {showModal && (
                <Modal closeModal={closeModal}>
                    <div>
                        <div className="text-left font-bold text-lg ">Edit {`${capitalizeFirstLetter(type)}`}</div>
                        <input
